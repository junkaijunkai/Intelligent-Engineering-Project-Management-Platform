...
import com.dahua.pvision.workflow.listener.strategy.ListenerFactory;
...
public class GlobalTaskAssigneeListener extends AbstractFlowableEngineEventListener {
    ...
    public static final String DETAIL_URL =
            "/workflow/process/detail/%s?definitionId=%s&deployId=%s&taskId=%s&finished=true";
    ...
    protected void taskAssigned(FlowableEngineEntityEvent event) {
        ...
        // 申请人id
        String createUid =
                taskEntity.getVariable(BpmnXMLConstants.ATTRIBUTE_EVENT_START_INITIATOR).toString();
        // 审批类型
     ...
     */
    public static void creatWxMessage(
            Long createUid,
            Long actinUid,
        ...
        if (StringUtils.isNotEmpty(taskWxName)) {
            ...
            if (ObjectUtil.isEmpty(variables)) {
                ...
                for (Map.Entry<String, Object> entry : variables.entrySet()) {
                    ...
                    if (!entry.getKey().equals(ProcessUtils.APPROVAL_TYPE)
                            && !entry.getKey().equals(INITIATOR)
                            && !entry.getKey().equals(ProcessUtils.TASK_DETAIL_URL_KEY)) {
            ...
            try {
                ...
                //                String processUrl = SsoUrlUtils.ssoCreate(appid, agentid, host +
                // path + SSO_URL + URLEncoder.encode(host + url, CharsetUtil.UTF_8));
                //                String taskDetailUrlEncode = SsoUrlUtils.ssoCreate(appid, agentid,
                // host + path + SSO_URL + URLEncoder.encode(taskDetailIUrl, CharsetUtil.UTF_8));
