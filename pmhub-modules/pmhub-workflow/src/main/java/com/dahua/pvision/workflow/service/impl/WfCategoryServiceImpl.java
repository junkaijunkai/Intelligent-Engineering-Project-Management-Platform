...
import com.dahua.pvision.workflow.domain.bo.WfCategoryBo;
import com.dahua.pvision.workflow.domain.vo.WfCategoryVo;
import com.dahua.pvision.workflow.factory.FlowServiceFactory;
import com.dahua.pvision.workflow.mapper.WfCategoryMapper;
import com.dahua.pvision.workflow.service.IWfCategoryService;
...
public class WfCategoryServiceImpl extends FlowServiceFactory implements IWfCategoryService {
    ...
    @Override
    public Table2DataInfo<WfCategoryVo> queryPageList(
            WfCategoryBo categoryBo, PageQuery pageQuery) {
    ...
    private LambdaQueryWrapper<WfCategory> buildQueryWrapper(WfCategoryBo categoryBo) {
        ...
        lqw.like(
                StringUtils.isNotBlank(categoryBo.getCategoryName()),
                WfCategory::getCategoryName,
                categoryBo.getCategoryName());
        lqw.eq(
                StringUtils.isNotBlank(categoryBo.getCode()),
                WfCategory::getCode,
                categoryBo.getCode());
        return lqw;
    }
            ...
            if (CollectionUtils.isNotEmpty(ids)) {
                ...
                if (CollectionUtils.isNotEmpty(errorIds)) {
                    throw new ServiceException(
                            "分类编码[" + String.join(",", errorIds) + "]" + "存在流程模型，不允许删除");
                }
            }
        }
    ...
    public String checkCategoryCodeUnique(String code) {
        List<WfCategory> wfCategories =
                baseMapper.selectList(
                        new LambdaQueryWrapper<WfCategory>().eq(WfCategory::getCode, code));
