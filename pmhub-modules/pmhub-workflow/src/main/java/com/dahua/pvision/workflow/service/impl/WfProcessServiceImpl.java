package com.dahua.pvision.workflow.service.impl;
...
import com.dahua.pvision.base.core.enums.ProjectTaskStatusEnum;
import com.dahua.pvision.base.core.exception.ServiceException;
import com.dahua.pvision.base.core.utils.DateUtils;
import com.dahua.pvision.base.core.utils.JsonUtils;
import com.dahua.pvision.base.core.utils.StringUtils;
import com.dahua.pvision.base.security.utils.SecurityUtils;
import com.dahua.pvision.workflow.common.constant.ProcessConstants;
import com.dahua.pvision.workflow.common.constant.TaskConstants;
import com.dahua.pvision.workflow.core.FormConf;
...
import com.dahua.pvision.workflow.domain.vo.*;
import com.dahua.pvision.workflow.domain.vo.*;
import com.dahua.pvision.workflow.factory.FlowServiceFactory;
import com.dahua.pvision.workflow.flow.FlowableUtils;
import com.dahua.pvision.workflow.mapper.WfCopyMapper;
import com.dahua.pvision.workflow.mapper.WfDeployFormMapper;
import com.dahua.pvision.workflow.mapper.WfMaterialsScrappedProcessMapper;
import com.dahua.pvision.workflow.mapper.WfTaskProcessMapper;
import com.dahua.pvision.workflow.service.IWfDeployService;
import com.dahua.pvision.workflow.service.IWfProcessService;
import com.dahua.pvision.workflow.service.IWfTaskService;
import com.dahua.pvision.workflow.utils.ModelUtils;
import com.dahua.pvision.workflow.utils.ProcessFormUtils;
import com.dahua.pvision.workflow.utils.ProcessUtils;
import com.dahua.pvision.workflow.utils.TaskUtils;
...
public class WfProcessServiceImpl extends FlowServiceFactory implements IWfProcessService {
    ...
    private final List<String> types =
            Arrays.asList(
                    "PURCHASE_INTO", "PURCHASE_OUT", "OTHER_INTO", "OTHER_OUT", "RETURN_INTO");
     ...
     */
    @Override
    public Table2DataInfo<WfDefinitionVo> selectPageStartProcessList(
            ProcessQuery processQuery, PageQuery pageQuery) {
        ...
        List<ProcessDefinition> definitionList =
                processDefinitionQuery.listPage(offset, pageQuery.getPageSize());
    ...
    @Override
    public Table2DataInfo<WfTaskVo> selectPageOwnProcessList(
            ProcessQuery processQuery, PageQuery pageQuery) {
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (Objects.nonNull(hisIns.getEndTime())) {
                taskVo.setDuration(
                        DateUtils.getDatePoor(hisIns.getEndTime(), hisIns.getStartTime()));
            } else {
                taskVo.setDuration(
                        DateUtils.getDatePoor(DateUtils.getNowDate(), hisIns.getStartTime()));
            }
            // 流程部署实例信息
            Deployment deployment =
                    repositoryService
                    ...
                    if (StringUtils.isNotBlank(task.getTaskId())) {
                        ...
                        if (t != null) {
                            if (StringUtils.isNotBlank(t.getAssignee())) {
                                SysUser sysUser =
                                        wfCopyMapper.selectUserById(Long.valueOf(t.getAssignee()));
    ...
    @Override
    public Table2DataInfo<WfTaskVo> selectPageTodoProcessList(
            ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        TaskQuery taskQuery =
                taskService
    ...
    @Override
    public Table2DataInfo<WfTaskVo> selectPageClaimProcessList(
            ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        TaskQuery taskQuery =
                taskService
    ...
    @Override
    public Table2DataInfo<WfTaskVo> selectPageFinishedProcessList(
            ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        ...
        for (HistoricTaskInstance histTask : historicTaskInstanceList) {
            ...
            flowTask.setDuration(
                    DateUtil.formatBetween(
                            histTask.getDurationInMillis(), BetweenFormatter.Level.SECOND));
    ...
    public String selectFormContent(String definitionId, String deployId) {
        ...
        WfDeployFormVo deployFormVo =
                deployFormMapper.selectVoOne(
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startTaskProcessByDefId(
            String taskId, String procDefId, String url, Map<String, Object> variables) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int startProjectProcessByDefId(
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startInboundProcessByDefId(
            String inboundId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startOutboundProcessByDefId(
            String outboundId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
        startOutboundProcess(outboundId, processDefinition, url, variables);
    }

    @Override
    public void startProviderProcessByDefId(
            String providerId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
     ...
     */
    @Override
    public void startOtherIntoProcessByDefId(
            String otherIntoId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
     ...
     */
    @Override
    public void startOtherOutProcessByDefId(
            String otherOutId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
     ...
     */
    @Override
    public void startReturnIntoProcessByDefId(
            String returnIntoId, String procDefId, String url, Map<String, Object> variables) {
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startScrappedProcessByDefId(
            String materialsIds, String procDefId, String url, Map<String, Object> variables) {
        ...
        //                errorIds.add(id);
        //            } else {
        //                if (!USELESS.equals(mus.getResolution()) ||
        // DAI_DING.equals(mus.getDangerous())) {
        //                    errorIds.add(id);
        //                }
        ...
        if (CollectionUtils.isNotEmpty(errorIds)) {
            throw new ServiceException(
                    "[" + String.join(",", errorIds) + "]" + "处理意见不是报废且危废待定，请重新选择之后发起审批");
        }
        if (CollectionUtils.isNotEmpty(eIds)) {
            throw new ServiceException("[" + String.join(",", eIds) + "]" + "不是未审核状态，请重新选择之后发起审批");
        }
        // 判断责任人是否为空
        //        List<MaterialsChangeRecords> materialsChangeRecords =
        // materialsChangeRecordsMapper.selectBatchIds(ids);
        ...
        //                throw new ServiceException("[" + msg.substring(0, msg.toString().length()
        // - 1) + "]" + "未设置责任人，请重新选择之后发起审批");
        //            }
        //        }
        ProcessDefinition processDefinition = getProcessDefinition(procDefId);
    ...
    private List<WfMaterialsScrappedProcess> getScrappedProcess(List<String> ids) {
        ...
        List<WfMaterialsScrappedProcess> scrappedProcessList =
                wfMaterialsScrappedProcessMapper.selectList(qw);
        if (CollectionUtils.isNotEmpty(scrappedProcessList)) {
            scrappedProcessList.forEach(
                    a -> {
                                ...
                                if (StringUtils.isBlank(
                                    ...
                                    List<Comment> comments =
                                            taskService.getProcessInstanceComments(
                                                    a.getInstanceId());
                                    comments.sort(
                                            Comparator.comparing(Comment::getTime).reversed());
        ...
        // 更新 MaterialsUseless
        //        LambdaUpdateChainWrapper<MaterialsUseless> updateWrapper = new
        // LambdaUpdateChainWrapper<>(materialsUselessMapper);
        //        updateWrapper.in(MaterialsUseless::getRecordId,
        // ids).set(MaterialsUseless::getApproved, 1);
        ...
        if (wfTaskProcess != null && StringUtils.isNotBlank(wfTaskProcess.getInstanceId())) {
            ...
            if (historicProcessInstance != null) {
                if (StringUtils.isBlank(historicProcessInstance.getEndActivityId())) {
                    ...
                    List<Comment> list =
                            taskService.getProcessInstanceComments(wfTaskProcess.getInstanceId());
            ...
            if (ProjectStatusEnum.PROJECT.getStatusName().equals(type)
                    || ProjectStatusEnum.TASK.getStatusName().equals(type)) {
                // 将任务状态改为进行中
                wfTaskProcessMapper.updateTaskStatus3(extraId);
            }
            //            if (types.contains(type)) {
            //                MaterialsChangeRecords materialsChangeRecords =
            // materialsChangeRecordsMapper.selectById(extraId);
            //                if (materialsChangeRecords.getProcessState() != 0) {
            //                    throw new ServiceException("[" + extraId + "]" +
            // "不是未审核状态，无需重新发起审批");
            //                }
            //            }
            // 新增
            wfTaskProcess =
                    deployService.insertWfTaskProcess(
                            extraId,
                            type,
                            materialsApprovalSetVO.getApproved(),
                            materialsApprovalSetVO.getDefinitionId(),
                            materialsApprovalSetVO.getDeploymentId());
        }
     ...
     * @return
     */
    private ProcessInstance startCommonProcess(
            ProcessDefinition procDef, String type, String url, Map<String, Object> variables) {
     ...
     * @param url
     */
    private void updateWfTaskProcess(
            WfTaskProcess wfTaskProcess, ProcessInstance processInstance, String url) {
     ...
     * @param url
     */
    private void updateScrappedProcess(
            List<WfMaterialsScrappedProcess> scrappedProcess,
            ProcessInstance processInstance,
            String url) {
        MaterialsApprovalSetVO materialsApprovalSetVO =
                deployService.queryApprovalSet(ProcessUtils.SCRAPPED_OUT_APPROVAL_TYPE, null);
        if (CollectionUtils.isNotEmpty(scrappedProcess)) {
            ...
            List<String> ids =
                    scrappedProcess.stream()
                            .map(WfMaterialsScrappedProcess::getMaterialId)
                            .collect(Collectors.toList());
            if (CollectionUtils.isNotEmpty(ids)) {
                LambdaUpdateChainWrapper<WfMaterialsScrappedProcess> updateWrapper =
                        new LambdaUpdateChainWrapper<>(wfMaterialsScrappedProcessMapper);
                updateWrapper
                        .in(WfMaterialsScrappedProcess::getMaterialId, ids)
                        .set(
                                WfMaterialsScrappedProcess::getApproved,
                                materialsApprovalSetVO.getApproved())
                        .set(WfMaterialsScrappedProcess::getType, materialsApprovalSetVO.getType())
                        .set(
                                WfMaterialsScrappedProcess::getDefinitionId,
                                materialsApprovalSetVO.getDefinitionId())
                        .set(
    ...
    private void queryLeaderId(Map<String, Object> variables, Long userId) {
        ...
        if (StringUtils.isNotBlank(sysUser.getLeaderId())) {
            // 直属上级
            variables.put(
                    ProcessUtils.LEADER_LIST, Arrays.asList(sysUser.getLeaderId().split(",")));
        }
    }
    ...
    private void startTaskProcess(
            String taskId, ProcessDefinition procDef, String url, Map<String, Object> variables) {
        // 任务审批相关逻辑
        ...
        ProcessInstance processInstance =
                startCommonProcess(procDef, ProcessUtils.TASK_APPROVAL_TYPE, url, variables);
    ...
    private void startProjectProcess(
        ...
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(projectId, ProcessUtils.PROJECT_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(procDef, ProcessUtils.PROJECT_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startInboundProcess(
            String inboundId,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(inboundId, ProcessUtils.PURCHASE_INTO_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(
                        procDef, ProcessUtils.PURCHASE_INTO_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startOutboundProcess(
            String outboundId,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(outboundId, ProcessUtils.PURCHASE_OUT_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(
                        procDef, ProcessUtils.PURCHASE_OUT_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
    ...
    private void startProviderProcess(
        ...
        ProcessInstance processInstance =
                startCommonProcess(procDef, "SUPPLIER_APPROVAL", url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startOtherIntoProcess(
            String otherIntoId,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(otherIntoId, ProcessUtils.OTHER_INTO_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(procDef, ProcessUtils.OTHER_INTO_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startOtherOutProcess(
            String otherOutId,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(otherOutId, ProcessUtils.OTHER_OUT_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(procDef, ProcessUtils.OTHER_OUT_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startReturnIntoProcess(
            String returnIntoId,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        WfTaskProcess wfTaskProcess =
                getWfTaskProcess(returnIntoId, ProcessUtils.RETURN_INTO_APPROVAL_TYPE);
        ProcessInstance processInstance =
                startCommonProcess(procDef, ProcessUtils.RETURN_INTO_APPROVAL_TYPE, url, variables);
        updateWfTaskProcess(wfTaskProcess, processInstance, url);
    }
     ...
     * @param variables
     */
    private void startScrappedProcess(
            List<String> ids,
            ProcessDefinition procDef,
            String url,
            Map<String, Object> variables) {
        ...
        ProcessInstance processInstance =
                startCommonProcess(
                        procDef, ProcessUtils.SCRAPPED_OUT_APPROVAL_TYPE, url, variables);
    ...
    private FormConf currTaskFormData(String deployId, HistoricTaskInstance taskIns) {
        ...
        if (ObjectUtil.isNotEmpty(deployFormVo)) {
            FormConf currTaskFormData =
                    JsonUtils.parseObject(deployFormVo.getContent(), FormConf.class);
    ...
    private List<FormConf> processFormList(BpmnModel bpmnModel, String procInsId, String deployId) {
        ...
        for (HistoricActivityInstance activityInstance : activityInstanceList) {
            ...
            FlowElement flowElement =
                    ModelUtils.getFlowElementById(bpmnModel, activityInstance.getActivityId());
            // 获取当前节点表单Key
            ...
            boolean localScope =
                    Convert.toBool(
                            ModelUtils.getElementAttributeValue(
            ...
            List<WfDeployFormVo> formInfoList =
                    deployFormMapper.selectVoList(
                            new LambdaQueryWrapper<WfDeployForm>()
                                    .eq(WfDeployForm::getDeployId, deployId)
            ...
            if (ObjectUtil.isNotNull(formInfo)) {
                ...
                String formName =
                        Optional.ofNullable(formInfo.getFormName()).orElse(StringUtils.EMPTY);
                String title =
                        localScope ? formName.concat("(" + flowElement.getName() + ")") : formName;
    ...
    @Deprecated
    private void buildUserTaskFormData(
            String procInsId, String deployId, Process process, List<FormConf> procFormList) {
        procFormList = procFormList == null ? new ArrayList<>() : procFormList;
        List<HistoricActivityInstance> activityInstanceList =
                historyService
                        .createHistoricActivityInstanceQuery()
        ...
        for (HistoricActivityInstance instanceItem : activityInstanceList) {
            UserTask userTask =
                    (UserTask) process.getFlowElement(instanceItem.getActivityId(), true);
    ...
    private List<WfProcNodeVo> historyProcNodeList(String procInsId) {
        ...
        for (HistoricActivityInstance activityInstance : historicActivityInstanceList) {
            ...
            } else if (BpmnXMLConstants.ELEMENT_TASK_USER.equals(
                    activityInstance.getActivityType())) {
                if (StringUtils.isNotBlank(activityInstance.getAssignee())) {
                    SysUser user =
                            wfCopyMapper.selectUserById(
                                    Long.parseLong(activityInstance.getAssignee()));
                    elementVo.setAssigneeId(user.getUserId());
                    elementVo.setAssigneeName(user.getNickName());
                }
                // 展示审批人员
    ...
    private WfViewerVo getFlowViewer(BpmnModel bpmnModel, String procInsId) {
        ...
        List<HistoricActivityInstance> finishedElementList =
                allActivityInstanceList.stream()
        ...
        Set<String> rejectedSet =
                FlowableUtils.dfsFindRejects(
                        bpmnModel, unfinishedTaskSet, finishedSequenceFlowSet, finishedTaskSet);
        return new WfViewerVo(
                finishedTaskSet, finishedSequenceFlowSet, unfinishedTaskSet, rejectedSet);
    }
}
