package com.dahua.pvision.workflow.listener;
...
import com.dahua.pvision.workflow.domain.vo.TaskCompletedStateVO;
import com.dahua.pvision.workflow.enums.TaskCompletedStateEnum;
import com.dahua.pvision.workflow.mapper.*;
import com.dahua.pvision.workflow.mapper.*;
...
public class GlobalProcessEndListener extends AbstractFlowableEngineEventListener {
    ...
    @Autowired private WfTaskProcessMapper wfTaskProcessMapper;

    @Autowired private WfTaskMessageDealMapper wfTaskMessageDealMapper;

    @Autowired private WfMaterialsScrappedProcessMapper wfMaterialsScrappedProcessMapper;
    //    @Autowired
    ...
    protected void processCompleted(FlowableEngineEntityEvent event) {
        ...
        SysUser sysUser =
                wfCopyMapper.selectUserById(
                        Long.parseLong(
                                processInstance
                                        .getVariable(
                                                BpmnXMLConstants.ATTRIBUTE_EVENT_START_INITIATOR)
                                        .toString()));
        ...
        // 最终结果
        TaskCompletedStateVO taskCompletedStateVO =
                listenerMapper.getLastTaskCompletedState(
                        flowableEntityEvent.getProcessInstanceId());
        ...
        if (TaskCompletedStateEnum.PASS.toString().equals(taskCompletedStateVO.getTaskTypeDesc())) {
            ...
            if (ObjectUtil.isNotEmpty(wfTaskProcess)) {
                if (ProjectStatusEnum.TASK.getStatusName().equals(type)) {
                    ...
                    //                    String id =
                    // wfTaskProcessMapper.selectLinkRecordsId(wfTaskProcess.getExtraId());
                    //                    MaterialsUseless materialsUseless =
                    // materialsUselessMapper.selectById(id);
                    //                    if (materialsUseless != null &&
                    // "报废".equals(materialsUseless.getResolution())) {
                    //                        throw new ServiceException("此申请所关联的入库单[" + id +
                    // "]正处于呆滞报废状态，库存已冻结，如需出库请联系管理员");
                    //                    }
                    //
                    // wfTaskProcessMapper.updateProcessState(wfTaskProcess.getExtraId());
                    //                    LogFactory.get().info("更新单据编号id:{}",
                    // wfTaskProcess.getExtraId());
            ...
            if (ProcessUtils.SCRAPPED_OUT_APPROVAL_TYPE.equals(type)) {
                //                LambdaQueryWrapper<WfMaterialsScrappedProcess> wrapper = new
                // LambdaQueryWrapper<>();
                //                wrapper.eq(WfMaterialsScrappedProcess::getInstanceId, instId);
                //                List<WfMaterialsScrappedProcess> scrappedProcessList =
                // wfMaterialsScrappedProcessMapper.selectList(wrapper);
                //                LogFactory.get().info("物料报废scrappedProcessList:{}",
                // JSON.toJSONString(scrappedProcessList));
                //                scrappedProcessList.forEach(scrappedProcess ->
                // materialsUselessAsyncTaskService.discardAllOrderAsUseless(Collections.singletonList(scrappedProcess.getMaterialId())));
                //                List<String> ids =
                ...
                // ids).set(MaterialsUseless::getApproved, 2);
                //                    updateWrapper.update();
                //                }
            }
        }
        if (TaskCompletedStateEnum.REJECT
                .toString()
                .equals(taskCompletedStateVO.getTaskTypeDesc())) {
            LambdaQueryWrapper<WfTaskProcess> qw = new LambdaQueryWrapper<>();
            qw.eq(WfTaskProcess::getInstanceId, instId).eq(WfTaskProcess::getType, type);
            WfTaskProcess wfTaskProcess = wfTaskProcessMapper.selectOne(qw);
            if (ObjectUtil.isNotEmpty(wfTaskProcess)) {
                if (!ProjectStatusEnum.TASK.getStatusName().equals(type)
                        && !ProcessUtils.SUPPLIER_APPROVAL_TYPE.equals(type)) {
            ...
            // LambdaUpdateChainWrapper<>(materialsUselessMapper);
            //                updateWrapper.in(MaterialsUseless::getRecordId,
            // ids).set(MaterialsUseless::getApproved, 0);
            //                updateWrapper.update();
            //            }
        ...
        if (StringUtils.isNotEmpty(createWxName)) {
            ...
            String url =
                    String.format(
                            "/workflow/process/detail/%s?definitionId=%s&deployId=%s&taskId=%s&finished=false",
                            instId, definitionId, deployId, taskId);
            // TODO: 2024.04.25 暂时关闭企微通知
            //            // 详情跳转地址
            //            String processUrl =  SsoUrlUtils.ssoCreate(appid,agentid,
            // host+path+"/sso/wx?url="+URLEncoder.encode(host+url));
            ...
            //                    ,processUrl
            //                    ,processUrl
            //                    ,taskCompletedStateVO.getTaskMessage(), sysUser.getUserName(),
            // listenerMapper.getProcessName(flowableEntityEvent.getProcessDefinitionId()) +
            // "流程结束提醒", "您申请的流程------" + taskCompletedStateVO.getTaskTypeDesc(),
            // OAUtils.ssoCreate(host + url)
            //            );
        ...
        List<WfTaskMessageDeal> wfTaskMessageDeals =
                wfTaskMessageDealMapper.selectList(queryWrapper);
        TaskCompletedStateVO finalTaskCompletedStateVO = taskCompletedStateVO;
        wfTaskMessageDeals.forEach(
                a -> {
                    // TODO: 2024.04.25  暂时关闭企微通知
                    //            RocketMqUtils.cleanMessage(a.getTaskId() + "_" + a.getAssignee());
                    //            OAUtils.restfulCall2(OAUtils.ALTER_MESSAGE_API,
                    // OAUtils.mapToStr(OAUtils.alterCustomMessageSingle(a.getTaskId() + "_" +
                    // a.getAssignee(), OAMessageStatusEnum.DEAL.getStatus(),
                    // wfCopyMapper.selectUserById(Long.valueOf(a.getAssignee())).getUserName())),
