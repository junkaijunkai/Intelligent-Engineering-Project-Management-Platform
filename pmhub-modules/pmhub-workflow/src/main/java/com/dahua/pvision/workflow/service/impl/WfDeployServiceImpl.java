...
import com.dahua.pvision.workflow.domain.WfApprovalSet;
...
import com.dahua.pvision.workflow.mapper.WfApprovalSetMapper;
...
public class WfDeployServiceImpl extends FlowServiceFactory implements IWfDeployService {
    ...
    @Override
    public Table2DataInfo<WfDeployVo> queryPageList(
    ...
    public void deleteByIds(List<String> deployIds) {
        for (String deployId : deployIds) {
            ...
            deployFormMapper.delete(
                    new LambdaQueryWrapper<WfDeployForm>().eq(WfDeployForm::getDeployId, deployId));
        }
    }
    ...
    public boolean updateApprovalSet(ApprovalSetDTO approvalSetDTO, String type) {
        ...
        if (ProjectStatusEnum.TASK.getStatusName().equals(type)) {
            queryWrapper
                    .eq(WfTaskProcess::getExtraId, approvalSetDTO.getTaskId())
                    .eq(WfTaskProcess::getType, ProjectStatusEnum.TASK.getStatusName());
        }
        if (ProjectStatusEnum.PROJECT.getStatusName().equals(type)) {
            queryWrapper
                    .eq(WfTaskProcess::getExtraId, approvalSetDTO.getProjectId())
                    .eq(WfTaskProcess::getType, ProjectStatusEnum.PROJECT.getStatusName());
        }
        WfTaskProcess pt = wfTaskProcessMapper.selectOne(queryWrapper);
        if (pt != null) {
            if ("1".equals(pt.getApproved())) {
                if (!Objects.equals(
                        wfTaskProcessMapper.selectStatusByTaskId2(approvalSetDTO.getTaskId()),
                        ProjectStatusEnum.NO_STARTED.getStatus())) {
    ...
    public boolean updateApprovalSet2(ApprovalSetDTO approvalSetDTO, String type) {
        LambdaQueryWrapper<WfApprovalSet> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper
                .eq(WfApprovalSet::getExtraId, approvalSetDTO.getTaskId())
                .eq(WfApprovalSet::getType, ProjectStatusEnum.TASK.getStatusName());
        ...
        if ("1".equals(wfApprovalSet.getApproved())) {
            ...
            qw.eq(WfTaskProcess::getExtraId, approvalSetDTO.getTaskId())
                    .eq(WfTaskProcess::getType, ProjectStatusEnum.TASK.getStatusName());
            ...
            if (pt != null && StringUtils.isNotBlank(pt.getInstanceId())) {
                ...
                HistoricProcessInstance historicProcessInstance =
                        historyService
                                ...
                                .processInstanceId(pt.getInstanceId())
                                .singleResult();
                if (historicProcessInstance != null) {
                    if (StringUtils.isBlank(historicProcessInstance.getEndActivityId())) {
                        ...
                        List<Comment> list =
                                taskService.getProcessInstanceComments(pt.getInstanceId());
    ...
    @Override
    public WfTaskProcess insertWfTaskProcess(
            String extraId,
            String type,
            String approved,
            String definitionId,
    ...
    @Override
    public boolean insertOrUpdateApprovalSet(
            String extraId,
            String type,
            String approved,
            String definitionId,
    ...
    @Override
    public List<WfMaterialsScrappedProcess> insertScrappedProcess(
            List<String> ids, MaterialsApprovalSetVO materialsApprovalSetVO) {
        List<WfMaterialsScrappedProcess> list = new ArrayList<>(10);
        if (CollectionUtils.isNotEmpty(ids)) {
            ids.forEach(
                    id -> {
                        WfMaterialsScrappedProcess wfMaterialsScrappedProcess =
                                new WfMaterialsScrappedProcess();
    ...
    public List<WfMaterialsScrappedProcess> selectScrappedList(List<String> ids) {
        List<WfMaterialsScrappedProcess> list = new ArrayList<>(10);
        if (CollectionUtils.isNotEmpty(ids)) {
            LambdaQueryWrapper<WfMaterialsScrappedProcess> queryWrapper =
