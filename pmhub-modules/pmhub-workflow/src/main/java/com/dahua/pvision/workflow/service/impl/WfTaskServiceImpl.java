...
import com.dahua.pvision.workflow.common.enums.FlowComment;
import com.dahua.pvision.workflow.domain.WfTaskMessageDeal;
import com.dahua.pvision.workflow.domain.bo.WfTaskBo;
import com.dahua.pvision.workflow.factory.FlowServiceFactory;
import com.dahua.pvision.workflow.flow.CustomProcessDiagramGenerator;
import com.dahua.pvision.workflow.flow.FlowableUtils;
import com.dahua.pvision.workflow.mapper.ListenerMapper;
import com.dahua.pvision.workflow.mapper.WfCopyMapper;
import com.dahua.pvision.workflow.mapper.WfTaskMessageDealMapper;
import com.dahua.pvision.workflow.mapper.WfTaskProcessMapper;
import com.dahua.pvision.workflow.service.IWfCopyService;
...
public class WfTaskServiceImpl extends FlowServiceFactory implements IWfTaskService {
    ...
    @Autowired ListenerMapper listenerMapper;
    @Autowired private WfTaskMessageDealMapper wfTaskMessageDealMapper;
    ...
    public void complete(WfTaskBo taskBo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            if (ObjectUtil.isNotEmpty(taskBo.getVariables())) {
                // 获取模型信息
                String localScopeValue =
                        ModelUtils.getUserTaskAttributeValue(
                                bpmnModel,
    ...
    public void taskReject(WfTaskBo taskBo) {
        ...
        List<Execution> executions =
                runtimeService.createExecutionQuery().parentId(task.getProcessInstanceId()).list();
    ...
    public void taskReturn(WfTaskBo bo) {
        ...
        Boolean isSequential =
                FlowableUtils.iteratorCheckSequentialReferTarget(
                        source, bo.getTargetKey(), null, null);
        if (!isSequential) {
        ...
        try {
            ...
            runtimeService
                    .createChangeActivityStateBuilder()
                    .processInstanceId(task.getProcessInstanceId())
                    .moveActivityIdsToSingleActivityId(currentIds, bo.getTargetKey())
                    .changeState();
    ...
    public void delegateTask(WfTaskBo bo) {
        ...
        //        OAUtils.restfulCall2(OAUtils.ALTER_MESSAGE_API,
        // OAUtils.mapToStr(OAUtils.alterCustomMessageSingle(bo.getTaskId() + "_" +
        // bo.getAssignee(), OAMessageStatusEnum.DEAL.getStatus(),
        // wfCopyMapper.selectUserById(SecurityUtils.getUserId()).getUserName())),
        // "OAUtils.ALTER_MESSAGE_API");
        LambdaQueryWrapper<WfTaskMessageDeal> qw = new LambdaQueryWrapper<>();
        qw.eq(WfTaskMessageDeal::getTaskId, bo.getTaskId())
                .eq(WfTaskMessageDeal::getAssignee, SecurityUtils.getUserId());
        wfTaskMessageDealMapper.delete(qw);
        StringBuilder commentBuilder =
                new StringBuilder(
                                wfCopyMapper
                                        .selectUserById(SecurityUtils.getUserId())
                                        .getNickName())
                        .append("->");
    ...
    private void extracted(String taskId) {
        ...
        //        OAUtils.restfulCall2(OAUtils.ALTER_MESSAGE_API,
        // OAUtils.mapToStr(OAUtils.alterCustomMessageSingle(taskId + "_" +
        // SecurityUtils.getUserId(), OAMessageStatusEnum.DEAL.getStatus(),
        // wfCopyMapper.selectUserById(SecurityUtils.getUserId()).getUserName())),
        // OAUtils.ALTER_MESSAGE_API);
        LambdaQueryWrapper<WfTaskMessageDeal> qw = new LambdaQueryWrapper<>();
        qw.eq(WfTaskMessageDeal::getTaskId, taskId)
                .eq(WfTaskMessageDeal::getAssignee, SecurityUtils.getUserId());
        wfTaskMessageDealMapper.delete(qw);
    }
    ...
    public void stopProcess(WfTaskBo bo) {
        ...
        // wfMaterialsScrappedProcessMapper.selectList(qw2);
        //        List<String> ids =
        // scrappedProcessList.stream().map(WfMaterialsScrappedProcess::getMaterialId).collect(Collectors.toList());
        //        LambdaUpdateChainWrapper<WfMaterialsScrappedProcess> updateWrapper = new
        // LambdaUpdateChainWrapper<>(wfMaterialsScrappedProcessMapper);
        ...
        //                .set(WfMaterialsScrappedProcess::getUpdatedBy,
    ...
    public void revokeProcess(WfTaskBo taskBo) {
        ...
        // 获取待撤回任务实例
        HistoricTaskInstance currTaskIns =
                historyService
        ...
        UserTask currUserTask =
                ModelUtils.getUserTaskByKey(bpmnModel, currTaskIns.getTaskDefinitionKey());
        // 查找下一级用户任务列表
        List<UserTask> nextUserTaskList = ModelUtils.findNextUserTasks(currUserTask);
        List<String> nextUserTaskKeys =
                nextUserTaskList.stream().map(UserTask::getId).collect(Collectors.toList());

        // 获取当前节点之后已完成的流程历史节点
        List<HistoricTaskInstance> finishedTaskInsList =
                historyService
                        .createHistoricTaskInstanceQuery()
                        .processInstanceId(procInsId)
        ...
        for (Task task : activateTaskList) {
            ...
            if (CollUtil.contains(nextUserTaskKeys, task.getTaskDefinitionKey())) {
                ...
                taskService.addComment(
                        task.getId(),
                        task.getProcessInstanceId(),
                        FlowComment.REVOKE.getType(),
                        wfCopyMapper.selectUserById(SecurityUtils.getUserId()).getNickName()
                                + "撤回流程审批");
        ...
        try {
            runtimeService
                    ...
                    .moveExecutionsToSingleActivityId(
                            revokeExecutionIds, currTaskIns.getTaskDefinitionKey())
                    .changeState();
        } catch (FlowableObjectNotFoundException e) {
            throw new RuntimeException("未找到流程实例，流程可能已发生变化");
        } catch (FlowableException e) {
        ...
        if (CollUtil.isNotEmpty(list)) {
            if (assignIds.isEmpty()) {
                ...
                for (String assignId : assignIds) {
                    Map<String, Object> assignVariables =
                            Collections.singletonMap(
                                    BpmnXMLConstants.ATTRIBUTE_TASK_USER_ASSIGNEE, assignId);
