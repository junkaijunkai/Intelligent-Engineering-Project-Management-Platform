package com.dahua.pvision.workflow.handler;
...
public class MultiInstanceHandler {
    ...
    @Autowired private WfCopyMapper wfCopyMapper;
                ...
                if ("ROLES".equals(dataType)) {
                    groups.forEach(
                            item -> {
                                List<String> userIds =
                                        wfCopyMapper
                                                .selectUserIdsByRoleId(Long.parseLong(item))
                                                .stream()
                                                .map(String::valueOf)
                                                .collect(Collectors.toList());
                    ...
                    List<String> userIds =
                            wfCopyMapper.selectUserIds(groups).stream()
                                    .map(String::valueOf)
                                    .collect(Collectors.toList());
                    candidateUserIds.addAll(userIds);
                }
