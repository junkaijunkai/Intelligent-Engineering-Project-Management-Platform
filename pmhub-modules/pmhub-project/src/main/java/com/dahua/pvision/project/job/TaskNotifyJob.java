package com.dahua.pvision.project.job;
...
import com.dahua.pvision.project.domain.ProjectTaskNotify;
import com.dahua.pvision.project.domain.vo.project.task.TaskNotifyDTO;
import com.dahua.pvision.project.mapper.ProjectTaskMapper;
import com.dahua.pvision.project.mapper.ProjectTaskNotifyMapper;
...
public class TaskNotifyJob {
    ...
    @Autowired private ProjectTaskNotifyMapper projectTaskNotifyMapper;
    ...
    public void taskNotify() {
        ...
        tasks.stream()
                .filter(
                        taskNotifyDTO ->
                                StringUtils.isNotBlank(taskNotifyDTO.getUserWxName())
                                        && taskNotifyDTO.getCloseTime() != null)
                .forEach(
                        taskNotifyDTO -> {
                            if (ProjectStatusEnum.PAUSE
                                    .getStatus()
                                    .equals(taskNotifyDTO.getStatus())) {
                                return;
                            }
                            LocalDate closeDate =
                                    taskNotifyDTO
                                            ...
                                            .toLocalDate();
                            if (ChronoUnit.DAYS.between(localDate, closeDate)
                                    == taskNotifyDTO.getNotifyDay()) {
                                log.info(
                                        "待逾期任务提醒开始, 用户id:{}, 企微id:{}, 任务id:{}",
                                        taskNotifyDTO.getUserId(),
                                        taskNotifyDTO.getUserWxName(),
                                        taskNotifyDTO.getTaskId());
                                // 进行待逾期消息提醒
                                // TODO: 2024.04.25 暂时注释掉逾期任务提醒功能
                                //                        TaskOverdueRemindDTO taskOverdueRemindDTO
                                // = new TaskOverdueRemindDTO();
                                //                        // 设置任务名称
                                //
                                ...
                                //                        // 设置任务详情地址
                                //                        String url = SsoUrlUtils.ssoCreate(appid,
                                // agentid, host + path + ssoPath + URLEncoder.encode(host +
                                // "/pmhub-project/my-task/info?taskId=" +
                                // taskNotifyDTO.getTaskId()));
                                //                        taskOverdueRemindDTO.setDetailUrl(url);
                                //
                                // taskOverdueRemindDTO.setOaTitle("任务即将逾期提醒");
                                //                        taskOverdueRemindDTO.setOaContext("您的任务【"
                                // + taskNotifyDTO.getTaskName() + "】还有【" +
                                // taskNotifyDTO.getNotifyDay() + "】天到期，请及时处理！");
                                //
                                // taskOverdueRemindDTO.setUserName(taskNotifyDTO.getUserName());
                                //
                                // taskOverdueRemindDTO.setLinkUrl(OAUtils.ssoCreate(host +
                                // "/pmhub-project/my-task/info?taskId=" +
                                // taskNotifyDTO.getTaskId()));
                                //
                                // RocketMqUtils.push2Wx(taskOverdueRemindDTO);
                                // 进行查询 如果数据库不存在记录 则就插入记录
                                LambdaQueryWrapper<ProjectTaskNotify> qw =
                                        Wrappers.lambdaQuery(ProjectTaskNotify.class)
                                                .eq(
                                                        ProjectTaskNotify::getTaskId,
                                                        taskNotifyDTO.getTaskId())
                                                .eq(ProjectTaskNotify::getOverdue, 0);
