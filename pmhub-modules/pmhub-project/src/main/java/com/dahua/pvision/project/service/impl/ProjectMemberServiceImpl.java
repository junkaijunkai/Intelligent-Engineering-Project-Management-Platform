...
import com.dahua.pvision.project.mapper.ProjectMemberMapper;
import com.dahua.pvision.project.service.ProjectMemberService;
import java.util.*;
...
public class ProjectMemberServiceImpl extends ServiceImpl<ProjectMemberMapper, ProjectMember>
    ...
    public void removeMemberList(ProjectVO projectVO) {
        ...
        queryWrapper
                .eq(ProjectMember::getPtId, projectVO.getProjectId())
    ...
    public PageInfo<ProjectMemberResVO> searchMember(ProjectMemberReqVO projectMemberReqVO) {
        ...
        // 根据项目id查询项目成员
        List<ProjectMemberResVO> projectMemberResVOList =
                projectMemberMapper.searchMember(projectMemberReqVO);
        // 查询用户信息
        List<Long> userIds =
                projectMemberResVOList.stream()
        ...
        projectMemberResVOList.forEach(
                projectMemberVO -> {
                    ...
                    if (Objects.nonNull(userVO)) {
                        ...
                        projectMemberVO.setRoleName(
                                !roles.isEmpty() ? roles.get(0).getRoleName() : "超级管理员");
                    }
                });
    ...
    public List<ProjectMemberResVO> queryUserList(String projectId, String keyword) {
        ...
        List<ProjectMemberResVO> memberResVOList =
                userVOList.stream()
                        .map(
                                userVO -> {
        ...
        Map<Long, List<ProjectMember>> map =
                projectMembers.stream().collect(Collectors.groupingBy(ProjectMember::getUserId));
        memberResVOList.forEach(
                a -> {
