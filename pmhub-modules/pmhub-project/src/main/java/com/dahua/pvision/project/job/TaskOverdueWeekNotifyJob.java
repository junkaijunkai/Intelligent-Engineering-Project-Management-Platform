...
import com.dahua.pvision.project.domain.ProjectTask;
...
public class TaskOverdueWeekNotifyJob {
    ...
    public void taskNotify() {
        ...
        projectTaskNotifies.forEach(
                projectTaskNotify -> {
                    if (ProjectStatusEnum.PAUSE
                            .getStatus()
                            .equals(
                                    projectTaskMapper.queryProjectStatus(
                                            projectTaskNotify.getProjectId()))) {
                        return;
                    }
                    LambdaQueryWrapper<ProjectTask> qw2 = new LambdaQueryWrapper<>();
                    qw2.eq(ProjectTask::getId, projectTaskNotify.getTaskId())
                    ...
                    if (projectTask != null && projectTask.getCloseTime() != null) {
                        ...
                        if (ChronoUnit.DAYS.between(localDate, closeDate) < 0L) {
                            if (!ProjectTaskStatusEnum.FINISHED
                                    .getStatus()
                                    .equals(projectTask.getExecuteStatus())) {
                                log.info(
                                        "已逾期任务每周重复提醒开始, 用户id:{}, 企微id:{}, 任务id:{}",
                                        projectTaskNotify.getUserId(),
                                        projectTaskNotify.getUserWxName(),
                                        projectTaskNotify.getTaskId());
                                //                        // 进行逾期任务消息提醒
                                //                        TaskOvertimeRemindDTO
                                // taskOvertimeRemindDTO = new TaskOvertimeRemindDTO();
                                //                        // 设置发送用户
                                //
                                ...
                                //                        taskOvertimeRemindDTO.setOaContext("您的任务【"
                                // + projectTaskNotify.getTaskName() + "】已经逾期，请及时处理！");
                                //
                                // taskOvertimeRemindDTO.setUserName(projectTaskMapper.queryUserName(projectTaskNotify.getUserId().longValue()));
                                //
                                // taskOvertimeRemindDTO.setLinkUrl(OAUtils.ssoCreate(host +
                                // "/pmhub-project/my-task/info?taskId=" +
                                // projectTaskNotify.getTaskId()));
                                //
