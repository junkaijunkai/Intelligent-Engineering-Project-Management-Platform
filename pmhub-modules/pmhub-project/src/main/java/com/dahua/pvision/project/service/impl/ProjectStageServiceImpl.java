...
import com.dahua.pvision.project.mapper.ProjectStageMapper;
...
public class ProjectStageServiceImpl extends ServiceImpl<ProjectStageMapper, ProjectStage>
    ...
    public List<ProjectStageVO> list(ProjectVO projectVO) {
        ...
        queryWrapper
                .eq(ProjectStage::getProjectId, projectVO.getProjectId())
        ...
        if (CollectionUtils.isNotEmpty(projectStages)) {
            ProjectStage ps =
                    projectStages.stream()
                            .max(Comparator.comparing(ProjectStage::getStageCode))
                            .get();
