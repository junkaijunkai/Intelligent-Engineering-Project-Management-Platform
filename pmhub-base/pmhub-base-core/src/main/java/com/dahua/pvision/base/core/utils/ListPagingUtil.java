...
public class ListPagingUtil {
    ...
    public static ListPagingUtil getListPagingUtil(
            Integer currentPage, Integer pageSize, List<?> list) {
        ...
        // 设置总页数
        pagingUtil.setPages(
                pagingUtil.getTotal() % pagingUtil.getPageSize() == 0
                        ? pagingUtil.getTotal() / pagingUtil.getPageSize()
                        : pagingUtil.getTotal() / pagingUtil.getPageSize() + 1);
        // 截取list
        pagingUtil.setList(
                list.subList(
                        pagingUtil.getStar(),
                        pagingUtil.getTotal() - pagingUtil.getStar() > pagingUtil.getPageSize()
