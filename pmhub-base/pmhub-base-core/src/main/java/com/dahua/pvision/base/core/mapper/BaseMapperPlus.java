package com.dahua.pvision.base.core.mapper;
...
import com.dahua.pvision.base.core.utils.BeanCopyUtils;
...
public interface BaseMapperPlus<M, T, V> extends BaseMapper<T> {
    ...
    default Class<V> currentVoClass() {
        return (Class<V>)
                ReflectionKit.getSuperClassGenericType(this.getClass(), BaseMapperPlus.class, 2);
    }
    ...
    default boolean insertBatch(Collection<T> entityList, int batchSize) {
        String sqlStatement =
                SqlHelper.getSqlStatement(this.currentMapperClass(), SqlMethod.INSERT_ONE);
        return SqlHelper.executeBatch(
                this.currentModelClass(),
                log,
                entityList,
                batchSize,
    ...
    default boolean updateBatchById(Collection<T> entityList, int batchSize) {
        String sqlStatement =
                SqlHelper.getSqlStatement(this.currentMapperClass(), SqlMethod.UPDATE_BY_ID);
        return SqlHelper.executeBatch(
                this.currentModelClass(),
                log,
                entityList,
    ...
    default boolean insertOrUpdateBatch(Collection<T> entityList, int batchSize) {
        ...
        return SqlHelper.saveOrUpdateBatch(
                ...
                (sqlSession, entity) -> {
                    ...
                    String sqlStatement =
                            SqlHelper.getSqlStatement(
                                    this.currentMapperClass(), SqlMethod.SELECT_BY_ID);
                ...
                (sqlSession, entity) -> {
                    ...
                    String sqlStatement =
                            SqlHelper.getSqlStatement(
                                    this.currentMapperClass(), SqlMethod.UPDATE_BY_ID);
                    sqlSession.update(sqlStatement, param);
                });
    }
    ...
    default boolean insertOrUpdate(T entity) {
        if (null != entity) {
            ...
            return StringUtils.checkValNull(idVal)
                            || Objects.isNull(selectById((Serializable) idVal))
                    ? insert(entity) > 0
                    : updateById(entity) > 0;
        }
        return false;
    ...
    /** 查询（根据ID 批量查询） */
    default <C> List<C> selectVoBatchIds(
            Collection<? extends Serializable> idList, Class<C> voClass) {
    ...
    default <C, P extends IPage<C>> P selectVoPage(
        ...
        IPage<C> voPage =
                new Page<>(pageData.getCurrent(), pageData.getSize(), pageData.getTotal());
