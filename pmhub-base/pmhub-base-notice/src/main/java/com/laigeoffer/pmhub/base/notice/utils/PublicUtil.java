...
/** 企业微信工具类 需要配置企微相关逻辑，如果配置有误，无法收到消息 */
@Component
public class PublicUtil {
    ...
    public static String getWorkWxToken() {
        ...
        JSONObject ipJson =
                JSONUtil.parseObj(
                        HttpRequest.get(String.format(WORK_WX_IP_URL, ACCESS_TOKEN))
                                .execute()
                                .body());

        if (ipJson.getInt(ERR_CODE_KEY_NAME) != 0) {
            String result2 =
                    HttpRequest.get(String.format(WORK_WX_URL, CORP_ID, CORP_SECRET))
                            .execute()
                            .body();
    ...
    public static JSONObject getWxUserBaseInfo(String code) {
        ...
        JSONObject ipJson =
                JSONUtil.parseObj(
                        HttpRequest.get(
                                        String.format(
                                                WORK_WX_USER_BASE_INFO, getWorkWxToken(), code))
                                .execute()
                                .body());
    ...
    public static JSONObject getWxUserDetailInfo(String ticket) {
        ...
        JSONObject userInfo =
                JSONUtil.parseObj(
                        HttpRequest.post(String.format(WORK_WX_USER_DETAIL_INFO, getWorkWxToken()))
