...
/** 泛微 OA 工具类 对接 OA 模块可直接使用 有需要帮助请联系公众号：苍何 */
@Slf4j
public class OAUtils {
    ...
    /** 授权地址 */
    private static final String OA_AUTHORIZE_URL =
            "/sso/oauth2.0/authorize?client_id=%s&response_type=code&redirect_uri=%s";

    /** 获取 access_token 接口 */
    private static final String OA_ACCESS_TOKEN_URL =
            "/sso/oauth2.0/accessToken?client_id=%s&client_secret=%s&grant_type=%s&code=%s&redirect_uri=%s";
    ...
    public static final String DELETE_MESSAGE_API =
            "/api/ec/dev/message/ deleteCustomMessageSingle";
    ...
    public static String ssoCreate(String target) {
        ...
        try {
            redirectUri =
                    URLEncoder.encode(
                            String.format(
                                    SERVER_PATH + OA_AUTHORIZE_URL,
                                    CLIENT_ID,
                                    URLEncoder.encode(host + path2 + ssoPath, UTF_8)
                                            + URLEncoder.encode(
                                                    URLEncoder.encode(target, UTF_8), UTF_8)),
                            UTF_8);
        ...
        try {
            redirectUri =
                    URLEncoder.encode(target, UTF_8)
                            + URLEncoder.encode(URLEncoder.encode(target2, UTF_8), UTF_8);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    public static String getOaAccessToken(String code, String redirectUri) {
        ...
        JSONObject jsonObject =
                JSONUtil.parseObj(
                        HttpRequest.post(
                                        String.format(
                                                SERVER_PATH + OA_ACCESS_TOKEN_URL,
                                                CLIENT_ID,
                                                CLIENT_SECRET,
                                                GRANT_TYPE,
                                                code,
    ...
    public static String getOAUserInfo(String accessToken) {
        ...
        JSONObject jsonObject =
                JSONUtil.parseObj(
                        HttpRequest.get(String.format(SERVER_PATH + OA_PROFILE_URL, accessToken))
                                .execute()
                                .body());
        if (jsonObject.getInt(ERROR_CODE) != NUM_0) {
    ...
    public static JSONObject logout(String service) {
        ...
        JSONObject ipJson =
                JSONUtil.parseObj(
                        HttpRequest.get(String.format(SERVER_PATH + OA_LOGOUT_URL, service))
                                .execute()
                                .body());
        return ipJson;
    }
     ...
     * 第一步：
     *
     * <p>调用ecology注册接口,根据appid进行注册,将返回服务端公钥和Secret信息
     */
    public static void testRegist(String address) {
        ...
        // 调用ECOLOGY系统接口进行注册
        String data =
                HttpRequest.post(address + REGISTER_API)
                        .header(APP_ID, APPID)
    ...
    public static Map<String, Object> testGetoken(String address) {
        ...
        // 调用ECOLOGY系统接口进行注册
        String data =
                HttpRequest.post(address + TOKEN_API)
                        .header(APP_ID, APPID)
    ...
    public static String testGetToken(String address) {
        ...
        String encryptSecret =
                rsa.encryptBase64(
                        StrUtil.nullToEmpty((String) map.get(SECRIT)),
     ...
     * 第三步：
     *
     * <p>调用ecology系统的rest接口，请求头部带上token和用户标识认证信息
     *
     * @param api rest api 接口地址(该测试代码仅支持GET请求)
     * @param jsonParams 请求参数json串
     *     <p>注意：ECOLOGY系统所有POST接口调用请求头请设置 "Content-Type","application/x-www-form-urlencoded;
     *     charset=utf-8"
     */
    public static JSONObject restfulCall(String api, String jsonParams, String type) {
        ...
        if (StringUtils.isNotBlank(type)) {
            return JSONUtil.parseObj(
                    HttpRequest.post(SERVER_PATH + api + jsonParams)
                            .header(APP_ID, APPID)
                            .header(TOKEN, token)
        ...
        JSONObject jsonObject =
                JSONUtil.parseObj(
                        HttpRequest.post(SERVER_PATH + api)
                                .header(APP_ID, APPID)
                                .header(TOKEN, token)
                                .header(SESSION, NUM_1)
                                .body(jsonParams)
    ...
    public static String mapToStr(Map<String, String> map) {
        ...
        if (map != null) {
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                if (entry.getValue() != null) stringBuilder.append("=").append(entry.getValue());
    ...
    public static Map<String, String> sendCustomMessageSingle(
            String loginId, String title, String context, String redirectUri, String businessId) {
    ...
    public static Map<String, String> alterCustomMessageSingle(
            String businessId, String state, String loginId) {
    ...
    public static Map<String, String> deleteCustomMessageSingle(
            Long userId, String state, String loginId) {
